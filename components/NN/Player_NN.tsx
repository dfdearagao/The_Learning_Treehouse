...
import { LANE_WIDTH, GameStatus } from '../../types_NN';
...
// Revised Geometries for "Cosmic Humanoid" look
// More organic shapes using Cylinders and Spheres instead of Boxes
const TORSO_GEO = new THREE.CylinderGeometry(0.22, 0.14, 0.55, 12); // Tapered torso
const HEAD_GEO = new THREE.SphereGeometry(0.18, 24, 24); // Smooth head
const ARM_GEO = new THREE.CylinderGeometry(0.06, 0.05, 0.55, 8); // Tapered arms
const LEG_GEO = new THREE.CylinderGeometry(0.08, 0.06, 0.7, 8); // Tapered legs
const HIPS_GEO = new THREE.SphereGeometry(0.15, 16, 16); // Ball hips

// Keep functional items but streamlined
const JETPACK_GEO = new THREE.BoxGeometry(0.2, 0.3, 0.08); // Slimmer jetpack
const GLOW_STRIP_GEO = new THREE.PlaneGeometry(0.03, 0.15);
const JOINT_SPHERE_GEO = new THREE.SphereGeometry(0.06, 12, 12);
...
export const Player: React.FC = () => {
  ...
  const { status, laneCount, takeDamage, hasDoubleJump, activateImmortality, isImmortalityActive, lives, maxLives, heal } = useStore();
  ...
  // Memoized Materials - Updated for Chrome/Iridescent look
  const { skinMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
      // High-visibility Neon Orange Skin
      const skinMaterial = new THREE.MeshPhysicalMaterial({
          color: isImmortalityActive ? '#ffffff' : '#ff9100',
          metalness: 0.8,
          roughness: 0.2,
          clearcoat: 1.0,
          emissive: isImmortalityActive ? '#00ffff' : '#ff4400',
      ...
      const shadowMaterial = new THREE.MeshBasicMaterial({ color: '#000000', opacity: 0.3, transparent: true });

      return { skinMaterial, jointMaterial, glowMaterial, shadowMaterial };
  }, [isImmortalityActive]);
  ...
  useFrame((state, delta) => {
    ...
    // Health Regeneration
    if (lives < maxLives && (status === GameStatus.PLAYING || status === GameStatus.SHOP)) {
        if (Date.now() - lastDamageTime.current > 10000) {
            heal(1);
            audio.playHeal();
  ...
  useEffect(() => {
     ...
     window.dispatchEvent(new Event('particle-burst')); // Dummy dispatch to keep typescript happy if needed, though 'player-hit' is custom
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      <group ref={bodyRef} position={[0, 1.1, 0]}> 
        ...
        {/* Torso */}
        <mesh castShadow position={[0, 0.2, 0]} geometry={TORSO_GEO} material={skinMaterial} />

        {/* Jetpack (Sleek) */}
        <mesh position={[0, 0.25, -0.15]} geometry={JETPACK_GEO} material={jointMaterial} />
        <mesh position={[-0.06, 0.25, -0.2]} geometry={GLOW_STRIP_GEO} material={glowMaterial} />
        <mesh position={[0.06, 0.25, -0.2]} geometry={GLOW_STRIP_GEO} material={glowMaterial} />
        ...
        <group position={[0.28, 0.4, 0]}>
            <group ref={rightArmRef}>
                ...
                <mesh position={[0, -0.52, 0]} geometry={JOINT_SPHERE_GEO} material={jointMaterial} />
            </group>
        </group>
        <group position={[-0.28, 0.4, 0]}>
        ...
        {/* Hips */}
        <mesh position={[0, -0.12, 0]} geometry={HIPS_GEO} material={skinMaterial} />
