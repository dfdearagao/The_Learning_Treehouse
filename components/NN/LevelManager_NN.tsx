...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, LEVEL_WORDS, NEON_PALETTE } from '../../types_NN';
import { audio } from '../System/Audio_NN';

// Geometry Constants
const OBSTACLE_HEIGHT = 1.6;
const OBSTACLE_GEOMETRY = new THREE.OctahedronGeometry(0.9, 0);
const OBSTACLE_GLOW_GEO = new THREE.OctahedronGeometry(0.95, 0);
const OBSTACLE_RING_GEO = new THREE.RingGeometry(0.8, 1.1, 4);
...
const getLetterInterval = (level: number) => {
    // Linear interval increase to avoid excessively long gaps at higher levels
    // Level 1: 150
    // Level 10: 150 + 450 = 600
    return BASE_LETTER_INTERVAL + (level - 1) * 50;
};
...
export const LevelManager: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isMenuReset || isRestart || isVictoryReset) {
        ...
        // Adjust next letter spawn for the new level's difficulty (Linear increase)
  ...
  useFrame((state, delta) => {
    ...
    if (furthestZ > -SPAWN_DISTANCE) {
         ...
         if (isLetterDue) {
             ...
             const currentWord = LEVEL_WORDS[level - 1] || "GALAXY";
             ...
             if (availableIndices.length > 0) {
                 ...
                 const color = NEON_PALETTE[chosenIndex % NEON_PALETTE.length];
    ...
    useFrame((state, delta) => {
        ...
        if (visualRef.current) {
            ...
            if (data.type === ObjectType.SHOP_PORTAL) {
                ...
                visualRef.current.rotation.x = Math.sin(state.clock.elapsedTime) * 0.2;
            } else {
                visualRef.current.position.y = baseHeight;
            }
        }
